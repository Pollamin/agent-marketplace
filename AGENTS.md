...
The core rule is: edit shared metadata and shared plugin content, then regenerate manifests. Do not hand-edit generated manifests as the source of truth.

## Repository Layout

- `marketplace.meta.json` is the source metadata for the marketplace itself.
- `plugins/<plugin>/plugin.meta.json` is the source metadata for an individual plugin.
...
- `scripts/sync_manifests.py` generates and checks all platform manifests.

## Development Workflow

After changing `marketplace.meta.json`, any `plugins/<plugin>/plugin.meta.json`, or plugin paths referenced by metadata, run:
...
There is no package manager or dependency install step for the current repo. The sync script uses only the Python standard library.

## Manifest Rules

- Treat `marketplace.meta.json` and `plugins/<plugin>/plugin.meta.json` as authoritative.
- Generated JSON is two-space indented with trailing newline and preserves key order from `scripts/sync_manifests.py`.
- Plugin component paths in `plugin.meta.json` must start with `./`.
...
1. Create `plugins/<plugin>/plugin.meta.json`.
2. Put reusable content under shared component paths such as `skills/`, `commands/`, `agents/`, `hooks/`, `assets/`, `.mcp.json`, or `.app.json`.
3. Reference only existing component paths from `plugin.meta.json`.
4. Run `python3 scripts/sync_manifests.py`.
5. Run `python3 scripts/sync_manifests.py --check`.
...
`pollamin-starter` is the starter plugin. Its useful content currently lives in `plugins/pollamin-starter/skills/repo-orientation/SKILL.md`, and its platform manifests are generated from `plugins/pollamin-starter/plugin.meta.json`.
